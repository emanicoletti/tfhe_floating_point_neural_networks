...
> {
    let (x_train, y_train, x_val, y_val, x_test, y_test) = match exp_number {
        1 | 2 => {
            let x_train: Array2<f32> =
                read_npy(Path::new("res/experiment_1_2/dataset/x_train.npy"))?;
            let y_train: Array2<f32> =
                read_npy(Path::new("res/experiment_1_2/dataset/y_train.npy"))?;
