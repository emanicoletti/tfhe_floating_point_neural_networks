...
impl PlainNeuralNetwork for PlainNeuralNetworkU32 {
    fn create(experiment: Option<i8>) -> Self {
        ...
        let inner = PlainNeuralNetworkImpl { layers, loss };
    ...
    fn add_dense(&mut self, input_size: usize, output_size: usize) {
        ...
        self.inner
            .add_dense(weights, biases, grad_weights, grad_biases);
    }
    ...
    fn add_conv(
        ...
        self.inner
            .add_conv(weights, biases, grad_weights, grad_biases, stride, padding);
    }

    fn add_batch_norm(&mut self, size: usize) {
        let (x_hat, mean, variance, gamma, beta) =
            self.init_batch_norm(&[size, self.get_depth()], self.experiment);
        self.inner
            .add_batch_norm(x_hat, mean, variance, gamma, beta);
    }
    ...
    fn train(
        ...
        self.inner.train(
            epochs,
            batch_size,
            u32_learning_rate.clone(),
            u32_weight_decay.clone(),
            u32_momentum.clone(),
            train_inputs.clone(),
            train_labels.clone(),
        );
            ...
            if layer.get_id() == id {
                ...
                if flat.len() != in_channels * out_channels * rows * cols {
                    println!(
                        ...
                        in_channels * out_channels * rows * cols,
                        flat.len()
                    );
                    return;
                }
                ...
                for c in 0..out_channels {
                    for k in 0..in_channels {
                        for i in 0..rows {
                            ...
                            for j in 0..cols {
                                let index =
                                    c * in_channels * rows * cols + k * rows * cols + i * cols + j;
...
impl PlainNeuralNetworkU32 {
    fn init_weights(
        ...
        if experiment == Some(1) {
            ...
            let weights_file: Array2<f32> = if output_size == 6272 && input_size == 256 {
                ...
                read_npy(Path::new(
                    "src/experiment_3/initializations/conv1_weight.npy",
                ))
                .expect("Failed to read conv1 weights")
            } else if in_channels == 32 && out_channels == 64 {
                read_npy(Path::new(
                    "src/experiment_3/initializations/conv2_weight.npy",
                ))
                .expect("Failed to read conv2 weights")
            } else if in_channels == 64 && out_channels == 128 {
                read_npy(Path::new(
                    "src/experiment_3/initializations/conv3_weight.npy",
                ))
                .expect("Failed to read conv3 weights")
            } else {
                panic!(
                    "No matching weight file for given layer dimensions {:?}, {:?}, {:?}, {:?}",
                    in_channels, out_channels, input_size, output_size
                );
            };
            ...
            let weights_file: Array2<f32> = if input_size == 200 && output_size == 784 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_1/fc1_weight.npy",
                ))
                .expect("Failed to load fc1 weights")
            } else if input_size == 10 && output_size == 200 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_1/fc2_weight.npy",
                ))
                .expect("Failed to load fc2 weights")
            } else {
                panic!(
                    "No matching weight file for given layer dimensions {:?}, {:?}, {:?}, {:?}",
                    in_channels, out_channels, input_size, output_size
            ...
            let weights_file: Array2<f32> = if in_channels == 1 && out_channels == 6 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_2/conv1_weight.npy",
                ))
                .expect("Failed to read conv1 weights")
            } else if in_channels == 6 && out_channels == 16 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_2/conv2_weight.npy",
                ))
                .expect("Failed to read conv2 weights")
            } else if input_size == 120 && output_size == 400 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_2/fc1_weight.npy",
                ))
                .expect("Failed to read fc1 weights")
            } else if input_size == 84 && output_size == 120 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_2/fc2_weight.npy",
                ))
                .expect("Failed to read fc2 weights")
            } else if input_size == 10 && output_size == 84 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_2/fc3_weight.npy",
                ))
            ...
            let weights_file: Array2<f32> = if in_channels == 3 && out_channels == 64 {
                read_npy(Path::new(
                    "src/skin_cancer_mnist/initializations_glyph/conv1_weight.npy",
                ))
                .expect("Failed to read conv1 weights")
            } else if in_channels == 64 && out_channels == 96 {
                read_npy(Path::new(
                    "src/skin_cancer_mnist/initializations_glyph/conv2_weight.npy",
                ))
                .expect("Failed to read conv2 weights")
            } else if input_size == 128 && output_size == 2400 {
                read_npy(Path::new(
                    "src/skin_cancer_mnist/initializations_glyph/fc1_weight.npy",
                ))
                .expect("Failed to read fc1 weights")
            } else if input_size == 7 && output_size == 128 {
                read_npy(Path::new(
                    "src/skin_cancer_mnist/initializations_glyph/fc2_weight.npy",
                ))
                .expect("Failed to read fc2 weights")
            } else {
                panic!(
            ...
            let weights_file: Array2<f32> = if in_channels == 3 && out_channels == 64 {
                read_npy(Path::new(
                    "src/blood_mnist/vgg_initializations/b1_conv1_w.npy",
                ))
                .expect("Failed to read conv1 weights")
            } else if in_channels == 64 && out_channels == 64 {
                read_npy(Path::new(
                    "src/blood_mnist/vgg_initializations/b1_conv2_w.npy",
                ))
                .expect("Failed to read conv2 weights")
            } else if in_channels == 64 && out_channels == 96 {
                read_npy(Path::new(
                    "src/blood_mnist/vgg_initializations/b2_conv1_w.npy",
                ))
                .expect("Failed to read conv3 weights")
            } else if in_channels == 96 && out_channels == 96 {
                read_npy(Path::new(
                    "src/blood_mnist/vgg_initializations/b2_conv2_w.npy",
                ))
            ...
            let weights_file: Array2<f32> = if in_channels == 3 && out_channels == 16 {
                read_npy(Path::new("src/cifar10/initializations/conv1_weight.npy"))
                    .expect("Failed to read conv1 weights")
            } else if input_size == 10 && output_size == 64 {
                read_npy(Path::new("src/cifar10/initializations/fc_weight.npy"))
                    .expect("Failed to read fc1 weights")
            } else {
                panic!(
                    "No matching weight file for given layer dimensions {:?}, {:?}, {:?}, {:?}",
            ...
            let weights_file: Array2<f32> = if input_size == 16 && output_size == 30 {
                read_npy(Path::new(
                    "src/breast_cancer/initializations/fc1_weight.npy",
                ))
                .expect("Failed to read fc1 weights")
            } else if input_size == 8 && output_size == 16 {
                read_npy(Path::new(
                    "src/breast_cancer/initializations/fc2_weight.npy",
                ))
                .expect("Failed to read fc2 weights")
            } else if input_size == 2 && output_size == 8 {
                read_npy(Path::new(
                    "src/breast_cancer/initializations/fc3_weight.npy",
                ))
                .expect("Failed to read fc3 weights")
            } else {
                panic!(
            ...
            let weights_file: Array2<f32> = if input_size == 128 && output_size == 784 {
                read_npy(Path::new("src/he_securenet/initializations/fc1_weight.npy"))
                    .expect("Failed to read fc1 weights")
            } else if input_size == 128 && output_size == 128 {
                read_npy(Path::new("src/he_securenet/initializations/fc2_weight.npy"))
                    .expect("Failed to read fc2 weights")
            } else if input_size == 10 && output_size == 128 {
                read_npy(Path::new("src/he_securenet/initializations/fc3_weight.npy"))
                    .expect("Failed to read fc3 weights")
            } else {
                panic!(
                    "No matching weight file for given layer dimensions {:?}, {:?}, {:?}, {:?}",
            ...
            let weights_file: Array2<f32> = if in_channels == 1 && out_channels == 5 {
                read_npy(Path::new(
                    "src/he_securenet/initializations/conv1_weight.npy",
                ))
                .expect("Failed to read conv1 weights")
            } else if input_size == 128 && output_size == 980 {
                read_npy(Path::new(
                    "src/he_securenet/initializations/cnn_fc2_weight.npy",
                ))
                .expect("Failed to read fc2 weights")
            } else if input_size == 10 && output_size == 128 {
                read_npy(Path::new(
                    "src/he_securenet/initializations/cnn_fc3_weight.npy",
                ))
                .expect("Failed to read fc3 weights")
            } else {
                panic!(
    ...
    fn init_biases(
        ...
        if experiment == Some(1) {
            ...
            let biases_file: Array2<f32> = if output_size == 6 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_2/conv1_bias.npy",
                ))
                .expect("Failed to read conv1 biases")
            } else if output_size == 16 {
                read_npy(Path::new(
                    "src/f_mnist_exp/initializations_2/conv2_bias.npy",
                ))
                .expect("Failed to read conv2 biases")
                ...
                panic!(
                    "No matching biases file for given layer dimensions {:?}",
                    output_size
                );
            };
            ...
            let biases_file: Array2<f32> = if output_size == 64 {
                read_npy(Path::new(
                    "src/skin_cancer_mnist/initializations_glyph/conv1_bias.npy",
                ))
                .expect("Failed to read conv1 biases")
            } else if output_size == 96 {
                read_npy(Path::new(
                    "src/skin_cancer_mnist/initializations_glyph/conv2_bias.npy",
                ))
                .expect("Failed to read conv2 biases")
            } else if output_size == 128 {
                read_npy(Path::new(
                    "src/skin_cancer_mnist/initializations_glyph/fc1_bias.npy",
                ))
                .expect("Failed to read fc1 biases")
            } else if output_size == 7 {
                read_npy(Path::new(
                    "src/skin_cancer_mnist/initializations_glyph/fc2_bias.npy",
                ))
                .expect("Failed to read fc2 biases")
            } else {
                panic!("No matching weight file for given layer dimensions");
            ...
            let biases_file: Array2<f32> = if input_size == 3 && output_size == 64 {
                read_npy(Path::new(
                    "src/blood_mnist/vgg_initializations/b1_conv1_b.npy",
                ))
                .expect("Failed to read conv1 biases")
            } else if input_size == 64 && output_size == 64 {
                read_npy(Path::new(
                    "src/blood_mnist/vgg_initializations/b1_conv2_b.npy",
                ))
                .expect("Failed to read conv2 biases")
            } else if input_size == 64 && output_size == 96 {
                read_npy(Path::new(
                    "src/blood_mnist/vgg_initializations/b2_conv1_b.npy",
                ))
                .expect("Failed to read conv3 biases")
            } else if input_size == 96 && output_size == 96 {
                read_npy(Path::new(
                    "src/blood_mnist/vgg_initializations/b2_conv2_b.npy",
                ))
            ...
            let biases_file: Array2<f32> = if output_size == 5 {
                ...
                read_npy(Path::new(
                    "src/he_securenet/initializations/cnn_fc2_bias.npy",
                ))
                .expect("Failed to read fc2 biases")
            } else if output_size == 10 {
                read_npy(Path::new(
                    "src/he_securenet/initializations/cnn_fc3_bias.npy",
                ))
                .expect("Failed to read fc3 biases")
            } else {
                panic!(
    ...
    fn init_batch_norm(
        ...
        let (beta_file, gamma_file): (Array2<f32>, Array2<f32>) = if experiment == Some(3) {
            if shape[0] == 32 {
                (
                    read_npy(Path::new("src/experiment_3/initializations/bn1_bias.npy"))
                        .expect("Failed to read batch norm 32"),
                    read_npy(Path::new("src/experiment_3/initializations/bn1_weight.npy"))
                        .expect("Failed to read batch norm 32"),
                )
            } else if shape[0] == 64 {
                (
                    read_npy(Path::new("src/experiment_3/initializations/bn2_bias.npy"))
                        .expect("Failed to read batch norm 64"),
                    read_npy(Path::new("src/experiment_3/initializations/bn2_weight.npy"))
                        .expect("Failed to read batch norm 64"),
                )
            } else if shape[0] == 128 {
                (
                    read_npy(Path::new("src/experiment_3/initializations/bn3_bias.npy"))
                        .expect("Failed to read batch norm 128"),
                    read_npy(Path::new("src/experiment_3/initializations/bn3_weight.npy"))
                        .expect("Failed to read batch norm 128"),
                )
            } else {
                panic!("No matching weight file for given layer dimensions");
            ...
            if shape[0] == 64 {
                (
                    read_npy(Path::new(
                        "src/skin_cancer_mnist/initializations_glyph/bn1_bias.npy",
                    ))
                    .expect("Failed to read batch norm 64"),
                    read_npy(Path::new(
                        "src/skin_cancer_mnist/initializations_glyph/bn1_weight.npy",
                    ))
                    .expect("Failed to read batch norm 64"),
                )
            } else if shape[0] == 96 {
                (
                    read_npy(Path::new(
                        "src/skin_cancer_mnist/initializations_glyph/bn2_bias.npy",
                    ))
                    .expect("Failed to read batch norm 96"),
                    read_npy(Path::new(
                        "src/skin_cancer_mnist/initializations_glyph/bn2_weight.npy",
                    ))
                    .expect("Failed to read batch norm 96"),
                )
            } else {
            ...
            if shape[0] == 64 && shape[1] == 1 {
                (
                    read_npy(Path::new(
                        "src/blood_mnist/vgg_initializations/b1_bn1_b.npy",
                    ))
                    .expect("Failed to read batch norm 64"),
                    read_npy(Path::new(
                        "src/blood_mnist/vgg_initializations/b1_bn1_w.npy",
                    ))
                    .expect("Failed to read batch norm 64"),
                )
                ...
                (
                    read_npy(Path::new(
                        "src/blood_mnist/vgg_initializations/b1_bn2_b.npy",
                    ))
                    .expect("Failed to read batch norm 64"),
                    read_npy(Path::new(
                        "src/blood_mnist/vgg_initializations/b1_bn2_w.npy",
                    ))
                    .expect("Failed to read batch norm 64"),
                )
                ...
                (
                    read_npy(Path::new(
                        "src/blood_mnist/vgg_initializations/b2_bn1_b.npy",
                    ))
                    .expect("Failed to read batch norm 96"),
                    read_npy(Path::new(
                        "src/blood_mnist/vgg_initializations/b2_bn1_w.npy",
                    ))
                    .expect("Failed to read batch norm 96"),
                )
                ...
                (
                    read_npy(Path::new(
                        "src/blood_mnist/vgg_initializations/b2_bn2_b.npy",
                    ))
                    .expect("Failed to read batch norm 96"),
                    read_npy(Path::new(
                        "src/blood_mnist/vgg_initializations/b2_bn2_w.npy",
                    ))
                    .expect("Failed to read batch norm 96"),
                )
            } else {
...
impl PlainNeuralNetwork for PlainNeuralNetworkU16 {
    ...
    fn train_and_validate(
        ...
        self.inner.train_and_validate(
            epochs,
            batch_size,
            f16::from_f32(learning_rate).to_bits(),
            f16::from_f32(weight_decay).to_bits(),
            f16::from_f32(momentum).to_bits(),
            train_inputs,
            train_labels,
            val_inputs,
            val_labels,
    ...
    fn init_weights(
        ...
        if experiment == Some(1) {
            if output_size == 16 {
                ...
                let fc2_weights = EXP1_W_FC2_32.to_vec();
                let flattened_fc2_weight: Vec<u16> = fc2_weights
                    .into_iter()
                    .flatten()
                    .map(|f| f16::from_f32(f32::from_bits(f)).to_bits())
                    .collect();
                ...
                let fc3_weights = EXP1_W_FC3_32.to_vec();
                let flattened_fc3_weight: Vec<u16> = fc3_weights
                    .into_iter()
                    .flatten()
                    .map(|f| f16::from_f32(f32::from_bits(f)).to_bits())
                    .collect();
