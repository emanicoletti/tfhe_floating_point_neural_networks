        ...
        + Default,
{
    fn forward(&mut self, input: &PlainTensor<T>) -> PlainTensor<T> {
        ...
        let mut padded_data =
            vec![T::default(); batch_size * in_channels * padded_height * padded_width];

        let pad_stride_c = padded_height * padded_width;
        let in_stride_c = in_height * in_width;

        padded_data
            .par_chunks_exact_mut(pad_stride_c)
            ...
            .for_each(|(pad_chan_slice, in_chan_slice)| {
                for h in 0..in_height {
                    ...
                    pad_chan_slice[dst_offset..dst_offset + in_width].copy_from_slice(src_row);
                }
            });

        let mut output_data =
            vec![T::default(); batch_size * out_channels * out_height * out_width];
        ...
        output_data
            .par_chunks_exact_mut(out_stride_image)
            .enumerate()
            .for_each(|(global_idx, out_image_slice)| {
                ...
                let weights_base_ptr =
                    unsafe { self.weights.data.as_ptr().add(oc * weight_stride_oc) };
                let input_base_ptr = unsafe { padded_data.as_ptr().add(b * pad_stride_b) };
    ...
    fn backward(&mut self, input: &PlainTensor<T>, grad_output: &PlainTensor<T>) -> PlainTensor<T> {
        ...
        padded_input_data
            .par_chunks_exact_mut(p_stride_c)
        ...
        grad_weights_data
            .par_chunks_exact_mut(w_stride_oc)
            ...
            .for_each(|(oc, (dw_slice, db_val))| {
                ...
                for b in 0..batch_size {
                    ...
                    let input_ptr_base =
                        unsafe { padded_input_data.as_ptr().add(input_batch_base) };
        ...
        grad_input_data
            .par_chunks_exact_mut(i_stride_c)
