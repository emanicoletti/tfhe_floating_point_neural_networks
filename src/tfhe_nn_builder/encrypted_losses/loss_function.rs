...
where
    K: ServerKeyTrait
        ...
        + EncryptedNegate<K, T>,
    T: Clone
        + FheTryEncrypt<<T as EncryptableValueType>::Plain, ClientKey>
        + EncryptedElement
        + EncryptableValueType,
{
    fn compute_loss(
        &self,
        predicted: &EncryptedTensor<T>,
        ...
        let n_enc =
            T::try_encrypt_plain(n_bits, &ctx.client_key).expect("Failed to encrypt float length");
