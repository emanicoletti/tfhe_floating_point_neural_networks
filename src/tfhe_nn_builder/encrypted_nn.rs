...
use tfhe::{
    ClientKey, CompressedServerKey, ConfigBuilder, CudaServerKey, FheUint16, FheUint32,
    set_server_key,
};
...
impl EncryptedNeuralNetwork for EncryptedNeuralNetworkU16GPU {
    ...
    fn add_dense(&mut self, input_size: usize, output_size: usize) {
        ...
        self.inner.add_dense(
            encrypted_weights,
            encrypted_biases,
            encrypted_grad_weights,
            encrypted_grad_biases,
        );
    }
    ...
    fn add_conv(
        ...
        let encrypted_weights = self.init_weights(
            kernel_width,
            kernel_height,
            in_channels,
            out_channels,
    ...
    fn add_tanh_activation(&mut self, size: usize) {
        ...
        self.inner
            .add_tanh_activation(derivatives, self.inner.context.ranges.clone());
    }
    ...
    fn train(
        ...
        let enc_learning_rate = FheUint16::try_encrypt(
            f16::from_f32(learning_rate).to_bits(),
            &self.inner.context.client_key,
        )
        .unwrap();
        ...
        self.inner.train(
            epochs,
            batch_size,
            enc_learning_rate.clone(),
            enc_train_inputs.clone(),
            enc_train_labels.clone(),
        );
        ...
        for layer in &self.inner.layers {
            if layer.get_id() == id {
                ...
                for i in 0..rows {
                    ...
                    for j in 0..cols {
                        let index = i * cols + j;
                        let decrypted: u16 = EncryptableValueType::decrypt(
                            &flat[index],
                            &self.inner.context.client_key,
                        );
    ...
    fn print_plain_biases(&self, id: String) {
        for layer in &self.inner.layers {
            if layer.get_id() == id {
                ...
                for i in 0..columns {
                    let decrypted: u16 =
                        EncryptableValueType::decrypt(&flat[i], &self.inner.context.client_key);
                    print!("{:<6} ", f16::from_bits(decrypted).to_f32());
                }
                print!("]\n");
                return;
...
impl EncryptedNeuralNetworkU16GPU {
    ...
    fn decrypt_tensor(&self, encrypted_tensor: &EncryptedTensor<FheUint16>) -> Vec<f32> {
        encrypted_tensor
            ...
            .iter()
            .map(|value| {
                f16::from_bits(EncryptableValueType::decrypt(
                    value,
                    &self.inner.context.client_key,
                ))
                .to_f32()
    ...
    fn init_weights(
        ...
        let mut encrypted_weights =
            Vec::with_capacity(input_size * output_size * in_channels * out_channels);

        for i in 0..(input_size * output_size * in_channels * out_channels) {
            let sample = plain_weights[i];
            let encrypted_sample = FheUint16::try_encrypt(sample, &self.inner.context.client_key)
                .expect("Weight initialization failed");
    ...
    fn init_biases(
        ...
        if experiment == Some(1) {
            if output_size == 4 {
                ...
                plain_biases = biases_u32
                    .iter()
                    .map(|&x| f16::from_f32(f32::from_bits(x)).to_bits())
                    .collect();
            } else if output_size == 2 {
    ...
    fn encrypt_dataset(
        ...
        for vec in clear_data {
            for sample in vec {
                ...
                let enc_sample =
                    FheUint16::try_encrypt(u16_sample, &self.inner.context.client_key).unwrap();
                encrypted_dataset.push(enc_sample);
            }
        }
        EncryptedTensor {
            data: encrypted_dataset,
            shape: vec![
                input_shapes[0],
                input_shapes[1],
                input_shapes[2],
...
impl EncryptedNeuralNetwork for EncryptedNeuralNetworkU32GPU {
    ...
    fn train(
        ...
        let enc_learning_rate =
            FheUint32::try_encrypt(learning_rate.to_bits(), &self.inner.context.client_key)
                .unwrap();
        let enc_train_inputs = self.encrypt_dataset(train_inputs, input_shapes.clone());
        let enc_train_labels = self.encrypt_dataset(train_labels, label_shapes.clone());
        let time = Instant::now();
                ...
                for c in 0..out_channels {
                    for r in 0..in_channels {
                        for i in 0..rows {
                            ...
                            for j in 0..cols {
                                let index =
                                    c * in_channels * rows * cols + r * rows * cols + i * cols + j;
                                let decrypted: u32 = EncryptableValueType::decrypt(
                                    &flat[index],
                                    &self.inner.context.client_key,
                                );
...
impl EncryptedNeuralNetworkU32GPU {
    ...
    fn decrypt_tensor(&self, encrypted_tensor: &EncryptedTensor<FheUint32>) -> Vec<f32> {
        encrypted_tensor
            ...
            .iter()
            .map(|value| {
                f32::from_bits(EncryptableValueType::decrypt(
                    value,
                    &self.inner.context.client_key,
                ))
            })
    ...
    fn init_weights(
        ...
        for i in 0..(input_size * output_size * in_channels * out_channels) {
            let sample = plain_weights[i];
            let encrypted_sample = FheUint32::try_encrypt(sample, &self.inner.context.client_key)
                .expect("Weight initialization failed");
            encrypted_weights.push(encrypted_sample);
        }
    ...
    fn encrypt_dataset(
        ...
        for vec in clear_data {
            for sample in vec {
                ...
                let enc_sample =
                    FheUint32::try_encrypt(u16_sample, &self.inner.context.client_key).unwrap();
                encrypted_dataset.push(enc_sample);
            }
        }
